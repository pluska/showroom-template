...
--      wrangler d1 execute showroom-db --local --file=./.users.local.sql
--
-- El correo se guarda SIEMPRE en minúsculas y sin espacios: el login normaliza
-- lo que se escribe antes de buscar (ver src/auth.ts), así que un correo
-- sembrado en mayúsculas no encontraría su fila y el usuario no podría entrar.
--
...
VALUES (
  ...
  'RELLENAR Nombre',
  'rellenar@ejemplo.com' -- en minúsculas,
